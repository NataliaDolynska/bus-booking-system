...
from booking.models import TaskProgress
from booking.utils.db_utils import if_exists
...
class GTFSLoader:
    ...
    def update_progress(self, message):
        if self.task_id:
            ...
            try:
                progress_record = if_exists(TaskProgress, task_id=self.task_id)
                if not progress_record:
                    logger.error(
                        f"TaskProgress.update_progress: TaskProgress does not exist for task_id {self.task_id}")
                    return
                ...
                logger.error(f"TaskProgress.update_progress: TaskProgress does not exist for task_id {self.task_id}", )
                raise ValueError(
                    f"TaskProgress.update_progress: TaskProgress does not exist for task_id {self.task_id}",
                    self.task_id)
